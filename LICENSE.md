# Styler Source-Available Commercial License
...
you're welcome to use Styler at no cost.
...
**Building a product with Styler?**
- If your product runs Styler for your users (like a code formatting SaaS),
  each user needs their own license.
- If you're just using Styler internally to keep your own codebase tidy,
  your customers don't need licenses - only your team does.

**About forks** - If someone forks Styler, commercial users still need to
...
**"Software"** means the Styler Java Code Formatter and all associated source
...
Examples:
- A SaaS code formatting service that uses Styler → each user needs a license
- A hosted IDE that invokes Styler for users → each user needs a license
- A CI/CD platform that runs Styler for customers → each customer needs a license

**Internal tooling exception**: If Your organization uses Styler solely for
internal development purposes (e.g., formatting Your own codebase during
build), and Your product does not invoke Styler at runtime for end users,
