...
if [[ "$TRIGGER_TYPE" == "SessionStart" ]]; then
  ...
  if [[ ${#PENDING_AUDITS[@]} -gt 0 ]]; then
    ...
    for AUDIT_INFO in "${PENDING_AUDITS[@]}"; do
      ...
      echo ""
      echo "    STEP 1: Automatically invoke process-recorder agent"
      echo "            Task tool (process-recorder) --task-name \"${TASK_NAME}\""
      echo ""
      echo "    STEP 2: Automatically invoke process-compliance-reviewer with trace output"
      echo "            Task tool (process-compliance-reviewer) --execution-trace \"{json from step 1}\""
      echo ""
      echo "    STEP 3a: IF process-compliance-reviewer verdict == PASSED:"
      ...
      echo ""
      echo "    STEP 3b: IF process-compliance-reviewer verdict == FAILED:"
  ...
  if [[ -n "$NEW_STATE" ]]; then
    ...
    if [[ "$CURRENT_STATE" != "$NEW_STATE" && "$CURRENT_STATE" != "unknown" ]]; then
      ...
      if [[ ! -f "$AUDIT_PASSED" ]]; then
        ...
        echo ""
        echo "STEP 1 (AUTOMATIC): Invoke process-recorder agent"
        echo "        Task tool (process-recorder) --task-name \"${TASK_NAME}\""
        echo "        Purpose: Collect objective facts about session execution"
        echo ""
        echo "STEP 2 (AUTOMATIC): Invoke process-compliance-reviewer with trace output"
        echo "        Task tool (process-compliance-reviewer) --execution-trace \"{json from step 1}\""
