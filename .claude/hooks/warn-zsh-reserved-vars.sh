...
# Fallback extraction if jq fails (shouldn't happen if jq is installed)
if [ -z "$TOOL_NAME" ]; then
	TOOL_NAME=$(extract_json_value "$JSON_INPUT" "tool_name")
fi

if [ -z "$COMMAND" ]; then
	# Note: This fallback is unreliable for multi-line commands with quotes
	# but kept for edge cases where jq might not be available
