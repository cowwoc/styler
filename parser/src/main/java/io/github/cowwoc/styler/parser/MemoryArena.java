...
public class MemoryArena
{
	...
	private int allocationsSinceLastCheck;

	/**
	 * Creates a memory arena with default capacity.
	 */
	public MemoryArena()
{
	...
	/**
	 * Creates a memory arena with the specified capacity.
	 *
	...
	public MemoryArena(int capacity)
{
     ...
     * @param parentId The parent node ID, or -{@code 1} for root
		...
		if (allocationsSinceLastCheck >= MEMORY_CHECK_INTERVAL)
{
     ...
     * @return the {@link NodeMetadata} for the specified node
     */
	...
	public void reset()
{
     ...
     *
     * @return the total number of nodes currently allocated in the arena
     */
	public int getNodeCount()
{
     ...
     *
     * @return the number of bytes currently used by allocated nodes
     */
		...
		if (usedMemory > MAX_MEMORY_USAGE_BYTES)
{
		...
		if (arenaMemory > maxArenaMemory * 0.9)
{ // 90% threshold
			throw new IllegalStateException(
