...
public class IndexOverlayParser implements AutoCloseable {
    ...
    private final JavaVersion targetJavaVersion;
     ...
     * @throws IllegalArgumentException if sourceText is null, empty, or exceeds size limits
     */
     ...
     * @throws IllegalArgumentException if input is null, empty, or exceeds size limits
     */
    private void validateInput(String sourceText) {
        ...
        if (sourceText.length() > MAX_SOURCE_LENGTH_CHARS) {
            throw new IllegalArgumentException(
                "Source text too large: " + sourceText.length() + " characters. " +
                "Maximum allowed: " + MAX_SOURCE_LENGTH_CHARS + " characters."
            );
        }

        // Check byte size when encoded as UTF-8 (approximate)
        int estimatedBytes = sourceText.length() * 3; // Conservative UTF-8 estimate
        if (estimatedBytes > MAX_SOURCE_SIZE_BYTES) {
            throw new IllegalArgumentException(
                "Source text too large: ~" + estimatedBytes + " bytes. " +
                "Maximum allowed: " + MAX_SOURCE_SIZE_BYTES + " bytes."
            );
        }

        // Validate that the input contains reasonable content
        if (sourceText.trim().isEmpty()) {
            throw new IllegalArgumentException("Source text cannot be empty or whitespace-only");
        }
    }
    ...
    private int parseTypeDeclaration(ParseContext context) {
        ...
        byte nodeType = switch (declType) {
            ...
            default -> throw new ParseException("Expected type declaration (class/interface/enum/record), found: " + declType);
        };
    ...
    private boolean isEnumConstant(ParseContext context) {
        ...
        // Simple cases: comma or semicolon clearly indicate enum constants
        if (isSimpleEnumConstantTerminator(next)) {
            return true;
        }

        // For LPAREN case, we need to distinguish between enum constant and constructor
        if (next == TokenType.LPAREN) {
            return isEnumConstantWithArguments(context);
        }
    ...
    private int findMatchingClosingParen(ParseContext context, int startIndex) {
        ...
        while (parenDepth > 0 && index < 20) { // Limit lookahead
    ...
    private boolean isLocalVariableDeclaration(ParseContext context) {
        ...
        if (context.currentTokenIs(TokenType.IDENTIFIER)) {
            ...
            if (next.type() == TokenType.LT) {
                ...
                while (angleDepth > 0 && index < 20) { // Limit lookahead to prevent infinite loops
    ...
    /**
     * Parse case patterns which can be simple expressions, type patterns, or null patterns.
     */
    ...
    private boolean isTypePattern(ParseContext context) {
        ...
        // Temporarily disable type pattern detection to debug
        return false;
        // return current.type() == TokenType.IDENTIFIER &&
        //        next.type() == TokenType.IDENTIFIER &&
        //        Character.isUpperCase(current.text().charAt(0)); // Simple heuristic: types start with uppercase
    }
