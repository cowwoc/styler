	...
	public NodeRegistry()
{
	 ...
	 * @throws IllegalArgumentException if initialCapacity is less than {@code 1}
	 */
	public NodeRegistry(int initialCapacity)
{
	 ...
	 * @param startOffset the starting position in the source
	 * @param length the length of the node in characters
	 * @param parentId the parent node ID, or {@code -1} for root nodes
	 * @return the unique identifier of the newly allocated node
	 */
		...
		if (parentId >= 0 && parentId < nodeCount)
{
	 ...
	 * @param newLength the new length value for the node
	 */
	public void updateNodeLength(int nodeId, int newLength)
{
	 ...
	 * @return the {@link NodeInfo} containing node data and relationships
	 */
	...
	public long getEstimatedMemoryUsage()
{
		...
		// 6 int arrays: startOffsets, lengths, parentIds, childrenStart, childrenCount
		return (capacity * 6 * Integer.BYTES) +
