...
/**
 * AST node representing a instanceofexpression.
 */
public final class InstanceofExpressionNode extends ASTNode
	{
	/**
	 * Creates an instanceof expression node.
	 *
	 ...
	 */
	public InstanceofExpressionNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<InstanceofExpressionNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new InstanceofExpressionNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link InstanceofExpressionNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<InstanceofExpressionNode>
		{
		...
		@Override
		public InstanceofExpressionNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new InstanceofExpressionNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
