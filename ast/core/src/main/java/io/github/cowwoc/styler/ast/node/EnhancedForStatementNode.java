...
/**
 * AST node representing a enhancedforstatement.
 */
public final class EnhancedForStatementNode extends ASTNode
	{
	/**
	 * Creates an enhanced for statement node.
	 *
	 ...
	 */
	public EnhancedForStatementNode(SourceRange range, List<Comment> leadingComments,
	...
	public <R, A> R accept(ASTVisitor<R, A> visitor, A arg)
		{
		return visitor.visitEnhancedForStatement(this, arg);
	}

	@Override
	public ASTNodeBuilder<EnhancedForStatementNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new EnhancedForStatementNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link EnhancedForStatementNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<EnhancedForStatementNode>
		{
		...
		@Override
		public EnhancedForStatementNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new EnhancedForStatementNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
