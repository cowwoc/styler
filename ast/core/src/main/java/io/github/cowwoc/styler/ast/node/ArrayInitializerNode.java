...
/**
 * AST node representing a arrayinitializer.
 */
public final class ArrayInitializerNode extends ASTNode
	{
	/**
	 * Creates an array initializer node.
	 *
	 ...
	 */
	public ArrayInitializerNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<ArrayInitializerNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new ArrayInitializerNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link ArrayInitializerNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<ArrayInitializerNode>
		{
		...
		@Override
		public ArrayInitializerNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new ArrayInitializerNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
