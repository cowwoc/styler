...
/**
 * AST node representing a newexpression.
 */
public final class NewExpressionNode extends ASTNode
	{
	/**
	 ...
	 */
	public NewExpressionNode(SourceRange range, List<Comment> leadingComments,
	...
	public <R, A> R accept(ASTVisitor<R, A> visitor, A arg)
		{
		return visitor.visitNewExpression(this, arg);
	}

	@Override
	public ASTNodeBuilder<NewExpressionNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new NewExpressionNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link NewExpressionNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<NewExpressionNode>
		{
		...
		@Override
		public NewExpressionNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new NewExpressionNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
