...
/**
 * AST node representing a patternmatch.
 */
public final class PatternMatchNode extends ASTNode
	{
	/**
	 * Creates a pattern match node.
	 *
	 ...
	 */
	public PatternMatchNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<PatternMatchNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new PatternMatchNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link PatternMatchNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<PatternMatchNode>
		{
		...
		@Override
		public PatternMatchNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new PatternMatchNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
