...
/**
 * AST node representing a breakstatement.
 */
public final class BreakStatementNode extends ASTNode
	{
	/**
	 ...
	 */
	public BreakStatementNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<BreakStatementNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new BreakStatementNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link BreakStatementNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<BreakStatementNode>
		{
		...
		@Override
		public BreakStatementNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new BreakStatementNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
