...
public final class QualifiedNameNode extends ASTNode {
	...
	public String getQualifiedName() { return String.join(".", parts); }

	@Override
	public <R, A> R accept(ASTVisitor<R, A> visitor, final A arg) { return visitor.visitQualifiedName(this, arg); }
	...
	public static final class Builder implements ASTNodeBuilder<QualifiedNameNode> {
		...
		public Builder setParts(List<String> parts) { this.parts = List.copyOf(parts); return this; }
		...
		@Override public Builder reset() { range = null; leadingComments = List.of(); trailingComments = List.of(); whitespace = WhitespaceInfo.none(); hints = FormattingHints.defaults(); parent = Optional.empty(); parts = List.of(); return this; }
		@Override public Builder copy() { return new Builder().setRange(range).setLeadingComments(leadingComments).setTrailingComments(trailingComments).setWhitespace(whitespace).setHints(hints).setParent(parent).setParts(parts); }
		@Override public boolean isValid() { return range != null && !parts.isEmpty(); }
		@Override public List<String> getValidationErrors() { var errors = new java.util.ArrayList<String>(); if (range == null) errors.add("range is required"); if (parts.isEmpty()) errors.add("parts cannot final be empty"); return errors; }
	}
}
