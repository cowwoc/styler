...
public final class RecordDeclarationNode extends ASTNode {
	...
	public static final class Builder implements ASTNodeBuilder<RecordDeclarationNode> {
		...
		public RecordDeclarationNode build() {
			...
			return new RecordDeclarationNode(range, leadingComments, trailingComments, whitespace, hints, parent, modifiers, name, typeParameters, parameters, interfaces, members);
		}

		@Override public Builder reset() { range = null; leadingComments = List.of(); trailingComments = List.of(); whitespace = WhitespaceInfo.none(); hints = FormattingHints.defaults(); parent = Optional.empty(); modifiers = List.of(); name = null; typeParameters = List.of(); parameters = List.of(); interfaces = List.of(); members = List.of(); return this; }
		@Override public Builder copy() { return new Builder().setRange(range).setLeadingComments(leadingComments).setTrailingComments(trailingComments).setWhitespace(whitespace).setHints(hints).setParent(parent).setModifiers(modifiers).setName(name).setTypeParameters(typeParameters).setParameters(parameters).setInterfaces(interfaces).setMembers(members); }
