...
/**
 * AST node representing a dowhilestatement.
 */
public final class DoWhileStatementNode extends ASTNode
	{
	/**
	 * Creates a do-while statement node.
	 *
	 ...
	 */
	public DoWhileStatementNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<DoWhileStatementNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new DoWhileStatementNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link DoWhileStatementNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<DoWhileStatementNode>
		{
		...
		@Override
		public DoWhileStatementNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new DoWhileStatementNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
