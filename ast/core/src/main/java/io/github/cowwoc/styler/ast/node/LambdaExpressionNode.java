...
/**
 * AST node representing a lambdaexpression.
 */
public final class LambdaExpressionNode extends ASTNode
	{
	/**
	 ...
	 */
	public LambdaExpressionNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<LambdaExpressionNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new LambdaExpressionNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link LambdaExpressionNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<LambdaExpressionNode>
		{
		...
		@Override
		public LambdaExpressionNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new LambdaExpressionNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
