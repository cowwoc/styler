...
public interface ASTNodeBuilder<T extends ASTNode> {
	 ...
	 * @return this builder for method chaining
	 */
	ASTNodeBuilder<T> reset();

	/**
	 * Creates a copy of this builder in its current state.
	 * @return a new builder with the same configuration
	 */
	ASTNodeBuilder<T> copy();

	/**
	 * Validates the current builder state without constructing a node.
	 * @return true if the builder state is valid for construction
	 */
	boolean isValid();
