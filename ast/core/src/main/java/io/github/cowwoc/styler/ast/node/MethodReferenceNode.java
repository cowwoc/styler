...
/**
 * AST node representing a methodreference.
 */
public final class MethodReferenceNode extends ASTNode
	{
	/**
	 * Creates a method reference node.
	 *
	 ...
	 */
	public MethodReferenceNode(SourceRange range, List<Comment> leadingComments,
	...
	public <R, A> R accept(ASTVisitor<R, A> visitor, final A arg)
		{
		return visitor.visitMethodReference(this, arg);
	}

	@Override
	public ASTNodeBuilder<MethodReferenceNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new MethodReferenceNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link MethodReferenceNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<MethodReferenceNode>
		{
		...
		@Override
		public MethodReferenceNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " + String.join(", ", getValidationErrors()));
			return new MethodReferenceNode(range, leadingComments, trailingComments, whitespace, hints,
				parent);
		}
