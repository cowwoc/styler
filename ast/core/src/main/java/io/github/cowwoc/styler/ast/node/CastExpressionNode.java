...
/**
 * AST node representing a castexpression.
 */
public final class CastExpressionNode extends ASTNode
	{
	/**
	 ...
	 */
	public CastExpressionNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<CastExpressionNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new CastExpressionNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link CastExpressionNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<CastExpressionNode>
		{
		...
		@Override
		public CastExpressionNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new CastExpressionNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
