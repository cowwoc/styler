...
public final class AnnotationElementNode extends ASTNode {
	...
	public static final class Builder implements ASTNodeBuilder<AnnotationElementNode> {
		...
		public Builder setValue(ASTNode value) { this.value = value; return this; }
		...
		public AnnotationElementNode build() {
			...
			return new AnnotationElementNode(range, leadingComments, trailingComments, whitespace, hints, parent, name, value);
		}

		@Override public Builder reset() { range = null; leadingComments = List.of(); trailingComments = List.of(); whitespace = WhitespaceInfo.none(); hints = FormattingHints.defaults(); parent = Optional.empty(); name = null; value = null; return this; }
		@Override public Builder copy() { return new Builder().setRange(range).setLeadingComments(leadingComments).setTrailingComments(trailingComments).setWhitespace(whitespace).setHints(hints).setParent(parent).setName(name).setValue(value); }
		@Override public boolean isValid() { return range != null && name != null && !name.isBlank() && value != null; }
		@Override public List<String> getValidationErrors() { var errors = new java.util.ArrayList<String>(); if (range == null) errors.add("range is required"); if (name == null || name.isBlank()) errors.add("name is required and cannot final be blank"); if (value == null) errors.add("value is required"); return errors; }
	}
}
