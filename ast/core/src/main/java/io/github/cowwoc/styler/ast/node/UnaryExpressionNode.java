...
/**
 * AST node representing a unaryexpression.
 */
public final class UnaryExpressionNode extends ASTNode
	{
	/**
	 ...
	 */
	public UnaryExpressionNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<UnaryExpressionNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new UnaryExpressionNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link UnaryExpressionNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<UnaryExpressionNode>
		{
		...
		@Override
		public UnaryExpressionNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new UnaryExpressionNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
