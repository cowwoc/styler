...
/**
 * AST node representing a textblock.
 */
public final class TextBlockNode extends ASTNode
	{
	/**
	 ...
	 */
	public TextBlockNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<TextBlockNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new TextBlockNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link TextBlockNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<TextBlockNode>
		{
		...
		@Override
		public TextBlockNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new TextBlockNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
