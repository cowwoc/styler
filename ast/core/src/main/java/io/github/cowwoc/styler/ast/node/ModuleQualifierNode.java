...
/**
 * AST node representing a module qualifier.
 *
 * @since 1.0
 */
public final class ModuleQualifierNode extends ASTNode
{
	private final List<ASTNode> children;

	/**
	 * Creates a module qualifier node.
	 *
	 ...
	 */
	public ModuleQualifierNode(SourceRange range, List<Comment> leadingComments,
	...
	{
		return visitor.visitModuleQualifier(this, arg);
	}

	@Override
	public ASTNodeBuilder<ModuleQualifierNode> toBuilder()
	{
		return new Builder().
			setRange(getRange()).
	...
	{
		return new ModuleQualifierNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent, children);
	}

	/**
	 * Builder for creating {@link ModuleQualifierNode} instances.
	 */
	public static final class Builder implements ASTNodeBuilder<ModuleQualifierNode>
	{
		...
		@Override
		public ModuleQualifierNode build()
		{
			if (!isValid())
			{
			...
			return new ModuleQualifierNode(range, leadingComments, trailingComments, whitespace,
