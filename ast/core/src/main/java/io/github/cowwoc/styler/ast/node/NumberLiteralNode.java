...
/**
 * AST node representing a numberliteral.
 */
public final class NumberLiteralNode extends ASTNode
	{
	/**
	 ...
	 */
	public NumberLiteralNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<NumberLiteralNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new NumberLiteralNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link NumberLiteralNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<NumberLiteralNode>
		{
		...
		@Override
		public NumberLiteralNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new NumberLiteralNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
