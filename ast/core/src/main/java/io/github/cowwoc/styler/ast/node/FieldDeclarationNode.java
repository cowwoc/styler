...
public final class FieldDeclarationNode extends ASTNode {
	...
	public List<ASTNode> getVariables() { return variables; }
	...
	public static final class Builder implements ASTNodeBuilder<FieldDeclarationNode> {
		...
		public Builder setVariables(List<ASTNode> variables) { this.variables = List.copyOf(variables); return this; }
		...
		public FieldDeclarationNode build() {
			...
			return new FieldDeclarationNode(range, leadingComments, trailingComments, whitespace, hints, parent, modifiers, type, variables);
		}

		@Override public Builder reset() { range = null; leadingComments = List.of(); trailingComments = List.of(); whitespace = WhitespaceInfo.none(); hints = FormattingHints.defaults(); parent = Optional.empty(); modifiers = List.of(); type = null; variables = List.of(); return this; }
		@Override public Builder copy() { return new Builder().setRange(range).setLeadingComments(leadingComments).setTrailingComments(trailingComments).setWhitespace(whitespace).setHints(hints).setParent(parent).setModifiers(modifiers).setType(type).setVariables(variables); }
		@Override public boolean isValid() { return range != null && type != null && !variables.isEmpty(); }
		@Override public List<String> getValidationErrors() { var errors = new java.util.ArrayList<String>(); if (range == null) errors.add("range is required"); if (type == null) errors.add("type final is required"); if (variables.isEmpty()) errors.add("variables cannot final be empty"); return errors; }
	}
}
