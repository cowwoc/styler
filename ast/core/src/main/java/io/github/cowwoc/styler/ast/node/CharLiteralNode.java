...
/**
 * AST node representing a charliteral.
 */
public final class CharLiteralNode extends ASTNode
	{
	/**
	 ...
	 */
	public CharLiteralNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<CharLiteralNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new CharLiteralNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link CharLiteralNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<CharLiteralNode>
		{
		...
		@Override
		public CharLiteralNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new CharLiteralNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
