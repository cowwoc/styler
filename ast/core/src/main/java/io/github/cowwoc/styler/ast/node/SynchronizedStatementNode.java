...
/**
 * AST node representing a synchronizedstatement.
 */
public final class SynchronizedStatementNode extends ASTNode
	{
	/**
	 * Creates a synchronized statement node.
	 *
	 ...
	 */
	public SynchronizedStatementNode(SourceRange range, List<Comment> leadingComments,
	...
	public <R, A> R accept(ASTVisitor<R, A> visitor, final A arg)
		{
		return visitor.visitSynchronizedStatement(this, arg);
	}

	@Override
	public ASTNodeBuilder<SynchronizedStatementNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new SynchronizedStatementNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link SynchronizedStatementNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<SynchronizedStatementNode>
		{
		...
		@Override
		public SynchronizedStatementNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new SynchronizedStatementNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
