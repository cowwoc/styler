...
/**
 * AST node representing a conditionalexpression.
 */
public final class ConditionalExpressionNode extends ASTNode
	{
	/**
	 ...
	 */
	public ConditionalExpressionNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<ConditionalExpressionNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new ConditionalExpressionNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link ConditionalExpressionNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<ConditionalExpressionNode>
		{
		...
		@Override
		public ConditionalExpressionNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new ConditionalExpressionNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
