...
/**
 * AST node representing a recordpattern.
 */
public final class RecordPatternNode extends ASTNode
	{
	/**
	 * Creates a record pattern node.
	 *
	 ...
	 */
	public RecordPatternNode(SourceRange range, List<Comment> leadingComments,
	...
	public <R, A> R accept(ASTVisitor<R, A> visitor, A arg)
		{
		return visitor.visitRecordPattern(this, arg);
	}

	@Override
	public ASTNodeBuilder<RecordPatternNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new RecordPatternNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link RecordPatternNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<RecordPatternNode>
		{
		...
		@Override
		public RecordPatternNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new RecordPatternNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
