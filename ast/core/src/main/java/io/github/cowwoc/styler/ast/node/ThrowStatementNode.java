...
/**
 * AST node representing a throwstatement.
 */
public final class ThrowStatementNode extends ASTNode
	{
	/**
	 * Creates a throw statement node.
	 *
	 ...
	 */
	public ThrowStatementNode(SourceRange range, List<Comment> leadingComments,
	...
	@Override
	public ASTNodeBuilder<ThrowStatementNode> toBuilder()
		{
	...
	protected ASTNode withMetadata(SourceRange newRange, List<Comment> newLeadingComments,
		...
		Optional<ASTNode> newParent)
			{
		return new ThrowStatementNode(newRange, newLeadingComments, newTrailingComments,
			newWhitespace, newHints, newParent);
	}

/**
 * Builder for creating {@link ThrowStatementNode} instances.
 */
	public static final class Builder implements ASTNodeBuilder<ThrowStatementNode>
		{
		...
		@Override
		public ThrowStatementNode build()
			{
			if (!isValid())
				throw new IllegalStateException("Invalid builder state: " +
					String.join(", ", getValidationErrors()));
			return new ThrowStatementNode(range, leadingComments, trailingComments, whitespace, hints, parent);
		}
