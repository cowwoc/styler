...
/**
 * Enforces execution timeouts for operations to prevent infinite loops
 * and pathological inputs from hanging the CLI tool.
 *
 * <p>This manager uses thread interruption to cancel operations that
 * exceed the configured timeout. Operations are executed in a separate
 * thread pool to enable clean cancellation.
 *
 * <h2>Example Usage:</h2>
 * <pre>{@code
...
public final class ExecutionTimeoutManager
{
	 ...
	 * If the operation exceeds the timeout, it is interrupted and an exception
	 * is thrown.
	 *
