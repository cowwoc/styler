...
public class IndexOverlayBenchmark
	...
	static class IndexOverlayAST
		...
		public int getStartOffset(int nodeId)
			{
			return startOffsets[nodeId]; }
		public int getLength(int nodeId)
			{
			return lengths[nodeId]; }
		public byte getNodeType(int nodeId)
			{
			return nodeTypes[nodeId]; }
		public int getParentId(int nodeId)
			{
			return parentIds[nodeId]; }
	...
	public static void compareMemoryFootprints(IndexOverlayAST indexAST, ObjectReferenceAST objectAST)
		...
		if (indexMemory < objectMemory)
		{
			efficiencyLabel = "better (Index-Overlay)";
		}
		else
		{
			efficiencyLabel = "better (Object-Reference)";
		}
